...
function solution(nums, target) { 
	...
	for (let i = 0; i < nums.length; i++) {
		if (map.some(e => e.key == target - nums[i])) {
			return map
				.filter(e => e.key == target - nums[i])
				.map(e => e.index)
				.concat(i);
		}
		else {
...
console.log(solution(list1, 5)); // [2, 6]

let list2 = [3, 2, 4];
console.log(solution(list2, 6)); // [1, 2]
...
console.log(solution(list3, 101)); // [83, 239]
